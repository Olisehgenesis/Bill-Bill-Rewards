...
contract BillBuddyRewards {
    ...
    struct GiftCard {
        ...
        uint256 pointCost;
        bool isActive;
        uint256 quality; // 0: Regular, 1: Silver, 2: Gold
    }
    ...
    function createGiftCard(
        ...
        require(_quality <= 2, "Invalid quality level");
        require(
    ...
    function mintRoyaltyGiftCard(uint256 _giftCardId) public {
        GiftCard storage giftCard = giftCards[_giftCardId];
        require(giftCard.isActive, "Gift card is not active");
        require(
            users[msg.sender].storePoints[giftCard.storeAddress] >=
                giftCard.pointCost,
            "Not enough points"
        );
        require(
            users[msg.sender].tier >= giftCard.quality,
            "Tier not high enough for this gift card"
        );

        uint256 pointsDeducted = giftCard.pointCost;
        users[msg.sender].storePoints[giftCard.storeAddress] -= pointsDeducted;
        users[msg.sender].totalPoints -= pointsDeducted;
        giftCard.isActive = false;

        require(
            cUsdToken.transfer(msg.sender, giftCard.value),
            "Transfer failed"
        );

        emit GiftCardAwarded(_giftCardId, msg.sender, pointsDeducted);
    }
    ...
    function listUserGiftCards(
        ...
        for (uint256 i = 1; i <= giftCardCounter && count < 10; i++) {
            if (
                giftCards[i].isActive &&
                users[_user].storePoints[giftCards[i].storeAddress] >=
                giftCards[i].pointCost
            ) {
